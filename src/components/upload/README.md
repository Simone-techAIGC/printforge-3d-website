...
5. **不要自己传 `onUpload`**：未传时默认已经走 `defaultUploader`（自动 `POST /api/storage/upload`，含进度/abort/鉴权/路径拼接）。除非业务确需走非默认上传协议（如直传 OSS、特殊鉴权头），**否则不要传 `onUpload`**，避免重复造轮且丢失默认能力。
...
// ❌ 5：手写 onUpload 重复造轮（默认已经有 defaultUploader，传了反而丢失路径拼接/鉴权/abort）
<ImageUpload
  onUpload={async (file) => {
...
| 字段           | 类型                                      | 说明                                                                                  |
| -------------- | ----------------------------------------- | ------------------------------------------------------------------------------------- |
| `onUpload` ⚠️  | `Uploader`                                | **默认不要传**。未传时自动走 `defaultUploader`（`POST /api/storage/upload`，含进度/abort/鉴权/路径拼接）；仅在需要走非默认上传协议（如直传 OSS、特殊鉴权头）时才自定义 |
...
>
> 图片预览 src 由 `useImagePreviewSrc(file)` 统一计算：优先 `response.url`（已拼为绝对地址），否则对原始 `file` 即时 `URL.createObjectURL`，自动 revoke。对空文件 / 非 `File` 实例都有兜底，不会白屏。
