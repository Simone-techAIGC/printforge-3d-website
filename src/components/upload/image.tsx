  ...
  return (
    <>
      ...
      {file.status === "error" && (
        <div
          className={cn(
            "absolute inset-0 flex flex-col items-center justify-center gap-2 bg-destructive/10 p-2 text-center text-xs text-destructive",
            classNames?.overlay,
          )}
        >
          <AlertCircle className="size-4" />
          <span className="line-clamp-2">{file.error ?? "上传失败"}</span>
          <Button
            type="button"
            size="sm"
            variant="secondary"
